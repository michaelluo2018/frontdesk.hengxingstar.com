...
$.ajax({
	type: "GET",
	url: "http://47.52.128.11:8090/anbi/backmgr/sysuser/getCustomList.do",
	data: {
		"token": _token,
		"pageSize": 10
	},
	...
	success: function(data) {
		...
		for(var i = 0; i < len; i++) {
			...
			result += '<tr><td class="uid">' + data.result.list[i].userId + '</td><td>' + data.result.list[i].userName + '</td><td class="reason">' + data.result.list[i].reason + '</td><td><input type="hidden" class="change_action_old" value="" /><select class="custom-select d-block change_action"> <option value="正常状态"'+status1+'>正常状态</option><option value="24小时冻结"'+status2+'>24小时冻结</option><option value="永远冻结"'+status3+'>永远冻结</option></select></td><td><a href="#" class="btn btn-secondary btn-sm update_user">修改</a></td></tr>';
			$(".list").append(result);		
